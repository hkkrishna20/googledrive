/*
 * package com.friends.in.appbapp;
 * 
 ...
 * import com.google.api.client.googleapis.media.MediaHttpDownloader; import
 * com.google.api.client.googleapis.media.MediaHttpUploader; import
 * com.google.api.client.http.FileContent; import
 * com.google.api.client.http.GenericUrl; import
 ...
 * com.google.api.client.util.Preconditions; import
 * com.google.api.client.util.store.DataStoreFactory; import
 ...
 * 
 * private static final String UPLOAD_FILE_PATH = "Enter File Path"; private
 * static final String DIR_FOR_DOWNLOADS = "Enter Download Directory"; private
 ...
 * secrets GoogleClientSecrets clientSecrets =
 * GoogleClientSecrets.load(JSON_FACTORY, new
 * InputStreamReader(DriveSample.class.getResourceAsStream(
 * "/client_secrets.json"))); if
 ...
 * System.exit(1); } // set up authorization code flow
 ...
 * dataStoreFactory) .build(); // authorize return new
 ...
 * dataStoreFactory = new FileDataStoreFactory(DATA_STORE_DIR); // authorization
 * Credential credential = authorize(); // set up the global Drive instance
 * drive = new Drive.Builder(httpTransport, JSON_FACTORY,
 * credential).setApplicationName(APPLICATION_NAME) .build();
 * 
 * // run commands
 * 
 * View.header1("Starting Resumable Media Upload"); File uploadedFile =
 * uploadFile(false);
 * 
 * View.header1("Updating Uploaded File Name"); File updatedFile =
 * updateFileWithTestSuffix(uploadedFile.getId());
 * 
 * View.header1("Starting Resumable Media Download"); downloadFile(false,
 * updatedFile);
 * 
 * View.header1("Starting Simple Media Upload"); uploadedFile =
 * uploadFile(true);
 * 
 * View.header1("Starting Simple Media Download"); downloadFile(true,
 * uploadedFile);
 * 
 * View.header1("Success!"); return; } catch (IOException e) {
 * System.err.println(e.getMessage()); } catch (Throwable t) {
 * t.printStackTrace(); } System.exit(1); }
 * 
/*
 * private static File uploadFile(boolean useDirectUpload) throws IOException {
 * File fileMetadata = new File(); fileMetadata.setTitle(UPLOAD_FILE.getName());
 * 
 ...
 * uploader.setProgressListener(new FileUploadProgressListener()); return
 * insert.execute(); }
 * 
/*
 * private static File updateFileWithTestSuffix(String id) throws IOException {
 * File fileMetadata = new File(); fileMetadata.setTitle("drivetest-" +
 * UPLOAD_FILE.getName());
 * 
 * Drive.Files.Update update = drive.files().update(id, fileMetadata); return
 * update.execute(); }
 * 
 *//**
	 ...
	 **//*
		 ...
		 * uploadedFile) throws IOException { // create parent directory (if necessary)
		 * java.io.File parentDir = new java.io.File(DIR_FOR_DOWNLOADS); if
		 * (!parentDir.exists() && !parentDir.mkdirs()) { throw new
		 * IOException("Unable to create parent directory" ); } OutputStream out = new
		 * FileOutputStream(new java.io.File(parentDir, uploadedFile.getTitle()));
		 * 
